...
- **Rate Limiting**: Request throttling and usage management
...
export async function POST(request: NextRequest) {
  ...
  // Check credits
  const cost = CREDIT_COSTS['api_call']
  const userCredits = await CreditService.getUserCredits(keyData.userId)
  ...
  try {
    ...
    return NextResponse.json({
      ...
      usage: {
        credits_used: cost,
        credits_remaining: (userCredits?.credits || 0) - cost
      }
    })
  } catch (error) {
